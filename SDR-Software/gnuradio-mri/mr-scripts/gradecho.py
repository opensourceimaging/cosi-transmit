...
execfile("gradecho_flowgraph.py")    # run flow graph
...
class pulse_params(object):
    ...
    def set_readgrad(self):
        ...
        grad_pre = -1*grad_pre/np.sum(grad_pre)*np.sum(grad_read)/2*self.prephasor_fudge
    ...
    def set_slicegrad(self):
        ...
        rewind_start = int(self.samp_rate*(self.p90+self.fudge*4))
                        
        length = rewind_start+slicegrad2.size-start
        data = np.ravel(np.zeros([length,1],float))
        
        data[0:size(slicegrad1)] = np.transpose(slicegrad1)
        data[-slicegrad2.size:] = -1*np.transpose(slicegrad2)
...
def show_pulses():
    T = 2*params.fudge+params.p90/2+params.TE+params.readout_time/2
