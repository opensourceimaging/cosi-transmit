...
class pulse_params(object):
    def __init__(self):        
        ...
        if(self.dead_time<0.00001):
            self.dead_time = 0.00001
        
        
    def set_pulses(self):
        ### Excitation Pulse ###
        exdata = self.power*pulse_shape.hard_pulse(self.p90,self.samp_rate)
        #exdata = self.power*pulse_shape.my_pulse()
    ...
    def set_dead_time(self,dead_time):
        if (dead_time < 0.00001):
            print "dead_time must be longer than 10 us!!!\n"
            print "dead_time was not changed."
            spec_gui.statusBar.showMessage("Dead Time must be longer than 10 us!!!", 3000)
